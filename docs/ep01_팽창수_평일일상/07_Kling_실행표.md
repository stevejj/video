...
## 1k. 2-02 실측 (2026-09-22) — ✅ 통과, 부위 하나 변화 검증
- 털뭉치만 움직이고 머리·눈·수건 고정. `Nothing else moves at all: his head stays..., his eyes keep..., wings and towel stay still` 열거 방식이 통함. 중간에 털뭉치 색이 짙어졌다 밝아짐(젖음→마름 해석, 허용).
- 3초=18, 108초 생성. 남은 클립 4개(3-02·5-02·9-02·10-02), 예상 84.

## 1l. 3-02 실측 (2026-09-22) — ✅ 통과, 회전 상한 검증
- 카메라 쪽 회전을 `to the three-quarter view of the end image, and no further` + 끝 프레임으로 제한 → 정면 과회전 없음. 모델이 스스로 넣은 긴 깜빡임(0.75~1.75s)은 졸림 연출에 맞아 허용.
- 3초=18, 246초 생성(대기열 길었음). 남은 클립 3개(5-02·9-02·10-02), 예상 66.

## 1m. 5-02 실측 (2026-09-22) — 조건부 통과, 보간 경로 교훈
- 기술 항목 전부 통과(부리 닫힘, 숟가락·그릇 고정, 첫·끝 일치). 이탈: 1.25~2.5s 눈이 완전히 뜨이고 카메라 응시.
...
## 1o. 10-02 실측 (2026-09-22) — ✅ 통과, 마지막 클립
- 눈꺼풀만 1.0~2.0s에 내려가 2.0s 완전 감김 후 유지. 머리·부리·램프·가장자리 전부 고정, 카메라 0.05px. 프레임간 변화 최대 0.88로 9클립 중 가장 조용함. `Nothing else moves at all: …` 열거 + 같은 눈 모양 계열 시작·끝 조합이 그대로 통함.
- 4초=24, 70초 생성. 크레딧 163.
- **전략 B 완료**: 시작+끝 9컷 중 7컷 클립 채택(1-02·2-01·2-02·3-02·6-01·9-01·9-02) + 10-02 기술 통과, 5-02는 정지 이미지. 정지 컷 13 + 9-03 + 5-02 = 15컷 정지 이미지. 총 소모 391 → 163 (228). 남은 163은 전략 C(정지 컷 승격) 또는 예비.
...
**프롬프트**
```
Pang Chang-su sits still at the table, holding the spoon motionless in his right wing above the pot, his cheeks slightly full. From the very first frame his happy, softly curved eyes slowly grow heavier and sleepier, smoothly over the whole clip, as after-lunch drowsiness sets in, until they exactly match the end image; that is the only thing that changes. Nothing else moves at all: his head stays at the same height and angle, his cheeks stay as they are, the spoon stays in his wing above the pot, and the stone pot, the side dishes, the open rice bowl and its lid stay exactly where they are. He does not turn his head toward the camera and never looks at the viewer. Static camera, locked-off tripod shot, no camera movement, same composition throughout. 3D animated plush character, short velvet fur; the surroundings and objects stay photoreal and completely unchanged. Beak stays closed the whole time. No other characters appear. No new objects appear. Wings stay short and rounded with no fingers.
```
...
**프롬프트**
```
From the very first frame Pang Mi-sun slowly turns her upper body and head to her right, toward the left side of the frame, smoothly over the whole clip, until she faces three-quarters to the left exactly as in the end image, and no further, looking toward her husband who is out of frame on the left. Her bright, cheerful, open eyes stay the same the whole time. Her beak stays closed for almost the whole clip and only parts by a hair at the very end, matching the end image; it never opens wide. Her lower body and feet stay in place, her head stays at the same height, and her wings stay at her sides. She keeps looking to the left and does not turn to face the camera. Her husband does not appear in the frame. Static camera, locked-off tripod shot, no camera movement, same composition throughout. 3D animated plush character, short velvet fur; the surroundings and objects stay photoreal and completely unchanged. No other characters appear. No new objects appear. Wings stay short and rounded with no fingers.
```
